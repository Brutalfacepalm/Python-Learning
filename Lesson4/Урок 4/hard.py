...
def withdraw_money(person, money):
    if person['money'] - money >= 0: # неверная логика, снимать не обязательно все, деньги на карте могут и оставатсья
        person['money'] -= money
...
def process_user_choice(choice, person):
    if choice == 1: # попытка сверить число со строкой
        print(check_account(person))
    elif choice == 2: # попытка сверить число со строкой
...
def start():
    ...
    while i <= 3:
        try:
            ...
            print('Неверный формат карты или пинкода(возможно лишние пробелы). Попыток осталось: {}'.format(3-i))
            i += 1
    if i >= 3:
